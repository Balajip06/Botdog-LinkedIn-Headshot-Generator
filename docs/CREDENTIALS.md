...
| ---------------------------- | ----------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `NEXT_PUBLIC_SITE_URL`       | yes                     | Your canonical site URL (e.g. `https://botdog-headshot.vercel.app`). For local dev, `http://localhost:3008` matches `.env.local.example:7`. | yes    | OG metadata + Stripe success/cancel URLs + push notification click-through URLs + sitemap base URL + `app/layout.tsx:21` `metadataBase` all silently fall back to `http://localhost:3000`. Sentry source-map upload pipeline expects a stable URL. |
