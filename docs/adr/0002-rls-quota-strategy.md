...
**No DB-level enforcement; rely on Row-Level Security policies + a "free tier" view.** Rejected: RLS policies can gate SELECT/UPDATE/DELETE cleanly but INSERT-time _aggregate_ checks (`count(*) >= 5`) are awkward in policy language. Triggers are the right tool.
