...
| `SITE_URL`                  | Your canonical app URL (e.g. `https://botdog-headshot.vercel.app`)                 | no                          | `dispatchNotification` silently no-ops. Generations still complete, but the push / email fallback after `status='completed'` never fires.                                                            |
