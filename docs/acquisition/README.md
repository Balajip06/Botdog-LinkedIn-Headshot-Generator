...
| **Referral automation** | $0 (existing schema + trigger) | ~1-2 h (Resend templates + ship) | +30-150 signups *if* there's an existing user base to refer-from | $0 cash, $0.50-1.50 per referred user (the bonus pack) | % of new signups with `referred_by` set; viral coefficient k |
...
Once one channel is *proven* (CAC stable, ROAS > 1x for paid OR k > 0.3 for organic), W3+ adds a second. That's how the channel mix gets built — sequentially, not in parallel.
