# Botdog — Sub-processor List

**Last updated:** 2026-05-29
**Canonical source:** This file.
**Linked from:** [`docs/PRIVACY_POLICY.md`](../PRIVACY_POLICY.md) §11 and [`docs/legal/DPA_TEMPLATE.md`](./DPA_TEMPLATE.md) Annex 3.

This page lists the sub-processors that Botdog engages to deliver the service. A sub-processor is any third party that processes personal data on behalf of Botdog in connection with the service. Each row identifies the sub-processor, what it is used for, what categories of data are shared with it, where it operates, and a link to its public privacy or DPA documentation.

---

## How we choose sub-processors

Botdog selects sub-processors that meet, at minimum, the following criteria:
...
- **Operational fit** — production-grade reliability, transparent pricing, and an incident-response posture that aligns with Botdog's 72-hour breach notification commitment to customers.
- **Data minimization** — Botdog forwards only the data each sub-processor strictly needs to deliver its function. Where a sub-processor offers optional features that would expand the data shared, Botdog defaults to off.

Vendor consolidation is preferred over fragmentation. Where one platform offers multiple capabilities (e.g., Supabase covers Database + Auth + Storage + Edge Functions), Botdog uses the consolidated offering to reduce the sub-processor count and audit surface.
...
- **Google (Gemini)** does not retain user photos or prompts when called via the paid API (per Google's published Gemini API terms). Botdog does not opt into any model-training data-sharing feature, so user inputs are processed in inference-only mode.
- **Stripe** receives card data directly from the user's browser via Stripe Checkout. Botdog's servers never see the raw card number, CVV, or expiry — only the Stripe-issued tokenized identifiers and the email/billing address Stripe forwards to us.
...
- **Cloudflare Turnstile** receives a transient browser fingerprint for bot-detection only. The fingerprint is hashed before any Botdog-side storage and is purged within 24 hours by the `pg_cron` job `purge_expired_anonymous`.

---

## Cross-border transfers

Several of the sub-processors above are based in the United States. For end users in the EEA, the UK, or Switzerland, Botdog relies on the following transfer mechanisms:

- **Standard Contractual Clauses (SCCs)** — incorporated into each sub-processor's DPA. Botdog has reviewed each sub-processor's published SCC implementation and accepts the standard text where the sub-processor offers it.
...
- **EU-US Data Privacy Framework** — where a US sub-processor is self-certified to the EU-US DPF, Botdog relies on that certification as the primary transfer mechanism, with SCCs as a fallback.

---

## Adding or removing a sub-processor

When Botdog adds, removes, or replaces a sub-processor, the following process applies:
...
3. **Notify customers** — for any customer who has a signed DPA with Botdog that incorporates Section 5.3 (Changes to Sub-processors), send an email notice at least **thirty (30) days** in advance of the change. The notice must include:
...
- [`docs/sops/incident_response.md`](../sops/incident_response.md) — the playbook when a sub-processor outage triggers an incident on Botdog's side.
