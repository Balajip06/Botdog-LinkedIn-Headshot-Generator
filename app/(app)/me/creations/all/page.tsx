...
export default async function AllCreationsPage({ searchParams }: PageProps) {
  ...
  if (MOCK_TRENDS_ENABLED) {
    ...
    if (!user) redirect('/login?next=/me/creations/all')
    ...
    // Escape LIKE wildcards so a query like "%_%" can't broaden the match
    // (RLS still scopes rows to this user, but keep the filter literal).
    if (rawQ) {
      const escaped = rawQ.replace(/[%_\\]/g, '\\$&')
      query = query.ilike('input_payload::text', `%${escaped}%`)
    }
  ...
  const buildViewHref = (nextView: 'all' | 'favorites'): string => {
    ...
    return qs ? `/me/creations/all?${qs}` : '/me/creations/all'
  }
    ...
    <section className="flex flex-col gap-8">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          >
            <ArrowLeft className="size-3.5" aria-hidden /> Account
          </Link>
          <h1 className="mt-2 text-4xl font-extrabold tracking-tight">
            All <span className="text-gradient-hero">creations</span>
          </h1>
        ...
        <GradientButton size="md" asChild>
          <Link href="/me/creations">New headshot</Link>
        </GradientButton>
      </header>
            ...
            <p className="text-muted-foreground mt-1 text-sm">
              {view === 'favorites'
                ...
                : rawQ || isFiltered
                  ...
                  : 'Make your first headshot in seconds.'}
            </p>
          </div>
