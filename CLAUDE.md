...
- **Runtime image model is global, not per-trend.** `app_settings.active_model` (singleton, `image_model` enum: `nano-banana` | `nano-banana-pro` | `gpt-image-2`) is the authority for customer + anonymous generation — read per-invocation by the `generate-image` Edge Function, which overrides `trends.model`. Admin flips it at `/admin/settings` (hot, no redeploy, **skips eval**). `trends.model` now drives only the admin eval preview (`runEval`). OpenAI (`gpt-image-2`) is wired **only** inside the Edge Function (`callOpenAI`, `images/edits`); `lib/image-provider/openai.ts` stays a stub. On failure the Edge tries the **other provider once** (`generateWithFallback`); per-**model** timeouts (gpt-image-2 85s — spike measured ~67s/call; Gemini 45s) keep the worst-case chain (~130s) under the ~150s wall, and `reap_stuck_processing` (pg_cron, 5 min) refunds any orphaned `processing` row. Needs `OPENAI_API_KEY` as an Edge secret. Keep the `MODEL_ID`/`COST_USD` maps in the Edge Function in sync with `lib/gemini/cost.ts` by hand (Deno can't import lib).
